...
public class GeocodingLocation extends Activity {
    ...
    public  void getLatitudeLocation(final String locationAddress,
                                        final Context context, final Handler handler) {
        Thread thread = new Thread() {
            public void run() {
                Geocoder geocoder = new Geocoder(context, Locale.getDefault());
                ...
                try {
                    ...
                    if (addressList != null && addressList.size() > 0) {
                       ...
                       Bundle latitudebundle = new Bundle();
                       latitudebundle.putString("latbun", String.valueOf(latt));
                       message.setData(latitudebundle);
                        message.sendToTarget();
